...
#Path
address<-paste("/Users/catarina/Desktop/DavidSoft/1stTestGUI/example-1-TNF_Monkeypox-rapid/ABCOutputs", sep="")
setwd(address)
############################
...
write(paste("Model selection with abc - cross validation - based on 10 samples",sep=""),"Results_text.txt",append=T)
try(capture.output(summary(cv.modsel), file = "Results_text.txt", append = TRUE), silent=TRUE)
write(paste("\n\n",sep=""),"Results_text.txt",append=T)
figureName1<-paste("./Results_ConfusionMatrix_10samp_",ThisName,".pdf",sep="")
pdf(figureName1)
try(plot(cv.modsel, names.arg=c("Blosum62", "Fitness", "Neutral")), silent=TRUE)
invisible(dev.off())
############################
...
pdf(figureName1)
res.gfit.Blosum62=suppressWarnings(gfit(target=FullRealmatrix, sumstat=FullSSmatrix[ModelsVector=="Blosum62",], statistic=mean, nb.replicate=10))
write(paste("   -Blosum62:",sep=""),"Results_text.txt",append=T)
capture.output(summary(res.gfit.Blosum62), file = "Results_text.txt", append = TRUE)
plot(res.gfit.Blosum62, main="Histogram under Blosum62")
res.gfit.Fitness=suppressWarnings(gfit(target=FullRealmatrix, sumstat=FullSSmatrix[ModelsVector=="Fitness",], statistic=mean, nb.replicate=10))
...
res.gfit.Neutral=suppressWarnings(gfit(target=FullRealmatrix, sumstat=FullSSmatrix[ModelsVector=="Neutral",], statistic=mean, nb.replicate=10))
...
############################
pca <- gfitpca(target=FullRealmatrix, sumstat=FullSSmatrix, index=ModelsVector, cprob=0.01)
#plot(pca, main="PCA")
#invisible(dev.off())
############################

############################
SS_All <- matrix(nrow=100)
for (i in 1:7){
  for(p in 1:3){
    SS <- FullSSmatrix[,i][(100 * (p-1) + 1):(100 * p)]
...
for (i in 1:7){
  for(p in 1:3){
    hist(SS_All[,(i * 3 + (-3 + p))], main=ModelsVector[100 * p], xlab=col_names_matrixSS[i])
...
for (i in 1:7){
  for(p in 1:3){
    SS <- SS_Abs[,(i * 3 - 2):(i * 3)][(100 * (p-1) + 1):(100 * p),]
    SS <- SS[order(SS[,2]),]
...
for (i in 1:7){
  for(p in 1:3){
    plot(y=SS_Tol[,(i * 3 * 2 + (-7 + p*2))], x=SS_Tol[,(i * 3 * 2 + (-6 + p*2))], main=ModelsVector[100 * p], ylab=col_names_matrixSS[i], xlab="Theta", ylim=c(MinMax[i*2-1], MinMax[i*2]))
