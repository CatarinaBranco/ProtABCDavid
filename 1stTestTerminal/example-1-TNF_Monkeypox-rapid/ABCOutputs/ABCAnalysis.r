...
#Path
address<-paste("/Users/catarina/Desktop/DavidSoft/1stTestTerminal/example-1-TNF_Monkeypox-rapid/ABCOutputs", sep="")
setwd(address)
############################
...
pdf(figureName1)
try(plot(cv.modsel, names.arg=c("Fitness", "HIVw", "Neutral")), silent=TRUE)
invisible(dev.off())
############################
...
res.gfit.HIVw=suppressWarnings(gfit(target=FullRealmatrix, sumstat=FullSSmatrix[ModelsVector=="HIVw",], statistic=mean, nb.replicate=10))
write(paste("   -HIVw:",sep=""),"Results_text.txt",append=T)
capture.output(summary(res.gfit.HIVw), file = "Results_text.txt", append = TRUE)
plot(res.gfit.HIVw, main="Histogram under HIVw")
